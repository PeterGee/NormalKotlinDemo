...
public class NetworkListener extends EventListener {
    ...
    public void callEnd(@NotNull Call call) {
        super.callEnd(call);
        NetLogUtils.d(TAG, "callEnd------requestId-----" + mRequestId);
    ...
    private void generateTraceData() {
        ...
        for (Map.Entry entry : traceList.entrySet()) {
            if (entry.getKey() == NetworkTraceBean.TRACE_NAME_TOTAL)
                NetLogUtils.d(TAG, "url===== "+traceModel.getUrl()+"   Time===  " + entry.getValue()+"ms");
        }
